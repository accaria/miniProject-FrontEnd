...
export default async function EventsListsBandung(){
   const bandungEvents : IEvent[]= await fetchAllEventService({location:"Bandung"});
    return(
        <>
        <div className="grid grid-cols-2 lg:grid-cols-3 space-y-10">
            {bandungEvents?bandungEvents.map((event,idx)=>(
                <div key={idx}>
